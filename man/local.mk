...
EXTRA_DIST += $(dist_man_MANS.1=.w) $(dist_man_MANS.1=.x) mancommon.x

# Each manpage depends on
# - its .w and .x files and its source script in bin
# - common.x for the SEE ALSO list
# - libAutom4teChannelDefs.pm which contains additional --help text
...
channeldefs_pm = libAutom4teChannelDefs.pm
man_common_dep = $(top_srcdir)mancommon.x 
		 $(top_srcdir)$(channeldefs_pm) 
		 $(top_srcdir).version 
		 $(top_srcdir)configure.ac

manautoconf.1   $(common_dep) manautoconf.w   manautoconf.x   $(binsrcdir)autoconf.in
manautoheader.1 $(common_dep) manautoheader.w manautoheader.x $(binsrcdir)autoheader.in
manautom4te.1   $(common_dep) manautom4te.w   manautom4te.x   $(binsrcdir)autom4te.in
manautoreconf.1 $(common_dep) manautoreconf.w manautoreconf.x $(binsrcdir)autoreconf.in
manautoscan.1   $(common_dep) manautoscan.w   manautoscan.x   $(binsrcdir)autoscan.in
manautoupdate.1 $(common_dep) manautoupdate.w manautoupdate.x $(binsrcdir)autoupdate.in
manifnames.1    $(common_dep) manifnames.w    manifnames.x    $(binsrcdir)ifnames.in
...
# build-auxhelp-extract.pl, which parses the source of the script
...
.w.1
	...
	$(HELP2MAN) 
	    --include=$(srcdir)$.x 
	    --include=$(srcdir)mancommon.x 
	    --source='$(PACKAGE_STRING)' 
	    --output=$@.t $(F)
	if $(SED) $(remove_time_stamp) $@ $@a.t 2devnull && 
	   $(SED) $(remove_time_stamp) $@.t  cmp $@a.t - devnull 2&1; then 
		touch $@; 
	else 
		mv $@.t $@; 
	fi
...
# We know we are in this case when 'mancommon.x' doesn't exist.
distclean-local distclean-local-man
distclean-local-man
	test -f mancommon.x  rm -f $(dist_man_MANS)